...
contract TestFunTokenPrecompileLocalGas {
    ...
    function callBankSend(uint256 amount, string memory bech32Recipient) public {
        uint256 sentAmount = FUNTOKEN_PRECOMPILE.sendToBank(erc20, amount, bech32Recipient);
        require(
            sentAmount == amount,
            string.concat(
    ...
    function callBankSendLocalGas(uint256 amount, string memory bech32Recipient, uint256 customGas) public {
        uint256 sentAmount = FUNTOKEN_PRECOMPILE.sendToBank{gas: customGas}(erc20, amount, bech32Recipient);
        require(
            sentAmount == amount,
            string.concat(
                "IFunToken.sendToBank succeeded but transferred the wrong amount",
